...
const SESSION_DIR = join(
  tmpdir(),
  "design-data-sessions-" + randomUUID().slice(0, 8),
);
const OUTPUT_DIR = join(
  tmpdir(),
  "design-data-output-" + randomUUID().slice(0, 8),
);
...
test("getSession throws for unknown session_id", (t) => {
  t.throws(() => getSession("no-such-session-" + randomUUID()), {
    message: /not found/,
  });
});
...
test("stepClassification updates wizard classification", (t) => {
  ...
  t.deepEqual(updated.wizard.classification.nameFields, [
...
test("stepValues updates wizard values", (t) => {
  const draft = startSession("/path");
  const rows = [
    { mode_combo: [], kind: "Literal", alias_target: "", literal: "#fff" },
  ];
